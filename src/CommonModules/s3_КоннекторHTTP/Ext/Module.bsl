...
// + #ЭДО СинхронизацияКонтрагентов Иванов А.Б. 21.01.2021
Функция сбл_ПрочитатьКакДвоичныеДанныеXML(ДвоичныеДанные, ВыдаватьСообщение = Истина, 
	ВыдаватьИсключение = Истина, ИмяСобытияДляЖР = Неопределено) Экспорт 
	...
	Исключение
		...
		ЕстьОшибка = Истина;
		СтрокаИсключения = НСтр("ru = 'Не удалось прочитать ДвоичныеДанные как XML ОписаниеОшибки %1'");
		СтрокаИсключения = СтрШаблон(СтрокаИсключения, ОписаниеОшибки());
	...
	Если ЕстьОшибка
		И ВыдаватьСообщение Тогда
		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрокаИсключения;
		Сообщение.Сообщить();
	...
	Если ЕстьОшибка
		И ВыдаватьИсключение Тогда
		ВызватьИсключение СтрокаИсключения;
	КонецЕсли;	
	
	Возврат ФайлОтветаXML;
	
КонецФункции // - #ЭДО СинхронизацияКонтрагентов Иванов А.Б. 21.01.2021

// + #ЭДО ВыгрузкаТиповогоДоговора Иванов А.Б. 26.01.2021
Функция сбл_ПрочитатьКакТекстXML(Ответ, 
	ВыдаватьСообщение = Истина, 
	ВыдаватьИсключение = Истина, 
	ИмяСобытияДляЖР = Неопределено) Экспорт
	...
	Исключение
		...
		ЕстьОшибка = Истина;
		СтрокаИсключения = НСтр("ru = 'Не удалось прочитать текст как XML ОписаниеОшибки %1'");
		СтрокаИсключения = СтрШаблон(СтрокаИсключения, ОписаниеОшибки());
	...
	Возврат ОтветXML;
КонецФункции // - #ЭДО ВыгрузкаТиповогоДоговора Иванов А.Б. 26.01.2021
...
// + #ЭДО ВыгрузкаТиповогоДоговора Иванов А.Б. 26.01.2021
Функция сбл_ПолучитьСоответствиеТиповMIME() Экспорт
	...
	Возврат мСоответствиеТиповMIME;
КонецФункции // - #ЭДО СинхронизацияКонтрагентов Иванов А.Б. 21.01.2021                                                                        
